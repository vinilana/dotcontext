...
Options: `[tool]` (omit to be prompted), `-g, --global` (default), `-l, --local`, `--dry-run`, `-v, --verbose`. See [Using dotcontext with MCP](/guides/using-with-mcp/) for the full list of supported tools and config paths.
