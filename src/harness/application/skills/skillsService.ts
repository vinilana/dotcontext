...
import { PHASE_NAMES_EN, type PrevcPhase } from '../../domain/workflow';
import { getOrBuildContext } from '../context/contextTools';
...
export class HarnessSkillsService {
  ...
  private getRegistry() {
    const { createSkillRegistry, BUILT_IN_SKILLS } = require('../../domain/workflow/skills');
  ...
  async scaffold(params: {
    ...
    const { createSkillGenerator } = require('../context/scaffolding/generators/skills');
  ...
  async export(params: {
    ...
    const { SkillExportService } = require('../exchange/export/skillExportService');
