...
3. Service processes and returns response`,
    },
    {
      ...
      exampleContent: '- **Harness**: Reusable runtime and domain (`src/harness/`)\n- **Adapters**: CLI, MCP, and host integrations (`src/cli/`, `src/mcp/`, `src/integrations/`)\n\n> Use `context({ action: "getMap", section: "all" })` for generated architecture and dependency summaries.',
      required: true,
      headingLevel: 2,
      defaultContent: `- **Entry Points**: Application entry and initialization (\`src/\`)
- **Harness**: Reusable runtime and domain (\`src/harness/\`)
