...
export class HarnessExecutionService {
  ...
  replaySession(
    ...
    return this.replay.replaySession(sessionId, options);
  }
  ...
  buildFailureDataset(options: BuildHarnessDatasetOptions = {}): Promise<HarnessFailureDataset> {
    return this.datasets.buildFailureDataset(options);
  }

  exportFailureDataset(sessionIds?: string[]): Promise<HarnessFailureDataset> {
    return this.buildFailureDataset({
      sessionIds,
