 ...
 *
 * Enforces the `cli -> harness <- mcp` boundary declared in AGENTS.md and
 ...
 * `src/harness/domain/workflow/index.ts` (or, better, a facade on
 * `WorkflowService`).
 *
 * If this test fails, the offending file is importing a workflow
 * submodule directly. Either:
 *   - Add the needed symbol to `src/harness/domain/workflow/index.ts`, or
...
// of these directly from `src/mcp/**` violates the boundary.
...
describe('mcp architectural boundary', () => {
  ...
  it('does not import workflow submodules directly', () => {
    ...
    if (violations.length > 0) {
      const message = violations
        ...
        .join('\n');
      throw new Error(
        `src/mcp must not import workflow submodules directly.\n` +
          `Use the workflow barrel or a ` +
